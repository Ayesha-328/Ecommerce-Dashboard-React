...
const OrderList = () => {
    ...
    const handleSearchTerm = (e) => {
        ...
        setSearchTerm(term);
        const filteredOrders = orderData.filter(order => order.id.toLowerCase().includes(term) || order.customer.toLowerCase().includes(term) || order.status.toLowerCase().includes(term))
    ...
    return (
        <motion.div className="bg-gray-800 backdrop-blur-md shadow-lg p-6 border border-gray-700 rounded-xl bg-opacity-50"
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="text-xl font-semibold text-gray-100">Orders List</div>
                <div className="relative">
                    <input type="text" placeholder="Search Orders..." onChange={handleSearchTerm} value={searchTerm} className="bg-gray-700 text-white placeholder-gray-400 rounded-lg pl-10 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    ...
                    <tbody className="divide-y divide-gray-700">
                        {filteredOrders.map((order) => (
                            <motion.tr key={order.id}
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-100 flex gap-2 items-center">
                                    {order.id}
                                </td>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                                    <span className={`text-sm px-2 inline-flex font-semibold rounded-full text-white 
